...
class Maze(object):
    def __init__(self, maze):
        self.maze = maze
        self.width   = int(len(maze[0])/2.)
        self.height  = int(len(maze)/2.)
        turtle.setworldcoordinates(-self.width, -self.height, self.width, self.height)
        self.blocks = []
        self.update_cnt = 0
        ...
        # Give hard coded electrode values
        # self.beacons = [(5, 0), (-5, 0), (0, 5), (0, -5)] 
    ...
    def is_in(self, x, y):
        if x < -self.width or y < -self.height or x > self.width or y > self.height:
            return False
        return True
    ...
    def is_free(self, x, y):
        ...
        xx = int(x)
        return self.maze[(yy+self.height)%(2*self.height)][(xx+self.width)%(2*self.width)] == 0
    ...
    def show_robot(self, robot):
        ...
        turtle.stamp()
        turtle.update()
        #print('Turtle initialized at')
        self.rob_pose = turtle.pos()
        #print(self.rob_pose)
