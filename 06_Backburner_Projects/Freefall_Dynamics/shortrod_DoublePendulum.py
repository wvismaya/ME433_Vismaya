...
q0 = 3*pi/4 - 0.1 # Initial configuration of pendulum
t0 = 0.0 # Initial time
tf = 10.0 # Final time
...
uMax = 15.0 # Max absolute input value
...
KStabilize = 2.*KVec[0] # Use only use first value to approximate infinite-horizon optimal controller gain
...
def simulateForward(tau, dsys, q0, xBar):
    ...
    x = dsys.xk
    fdx = dsys.fdx()
    xTilde = np.array([wrapToPi(x[0] - xBar[0]), wrapToPi(x[1]), x[2], x[3] ])
    ...
    while mvi.t1 < tf-dt:
        if mvi.t1 < tau:
            if x[2] == 0:
                ...
                u = np.array([-x[2]*KEnergy*eTilde]) # Swing-up controller
        else:
        ...
        eTilde = e - eBar
        l = cost.l(np.array([wrapTo2Pi(x[0]), wrapToPi(x[1]), x[2], x[3] ]), u, k)
...
while cnt < 10 and abs(JdTau) > .001:
    ...
    eTilde = E[k] - eBar
    u2 = np.array([-x[2]*KEnergy*eTilde])
...
pylab.plot(T, np.array(X)[:,0])
#pylab.plot(T, np.array(X)[:,1])
pylab.plot(T, qBar*np.ones(len(T)))
pylab.title("Swing-up and stabilize with optimal switching time")
pylab.ylabel("X")
#pylab.legend(["theta","p"])
