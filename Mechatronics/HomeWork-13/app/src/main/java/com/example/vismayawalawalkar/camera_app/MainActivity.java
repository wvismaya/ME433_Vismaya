package com.example.vismayawalawalkar.camera_app;
...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        for(int ii = 0; ii <bmp.getHeight(); ii++) {
            if (c != null) {
                int thresh = 0; // comparison value
                int[] pixels = new int[bmp.getWidth()]; // pixels[] is the RGBA data
                int startY = ii; //200; // which row in the bitmap to analyze to read
                bmp.getPixels(pixels, 0, bmp.getWidth(), 0, startY, bmp.getWidth(), 1);

                // in the row, see if there is more green than red
                thresh = progressChanged;
                for (int i = 0; i < bmp.getWidth(); i++) {
                    if (((green(pixels[i]) - red(pixels[i])) > thresh) && (((green(pixels[i]) - blue(pixels[i])) > 0.5 * thresh))) {
